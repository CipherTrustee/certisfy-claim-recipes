...
async function validateClaim(claim,action,ttl){
      const verification = await certisfyCrypto.verifyClaim(claim, "ppbv.certisfy.com", claim.trustChain);
  
      //for ppbv we'll also allow demo trust anchor certificates
      if(!certisfyCrypto.isClaimTrustworthy(verification) && (certisfyCrypto.buildErrorList(verification).length >1 || !verification.certChainVerification || !verification.certChainVerification.chain.find(e=>e.finger_print == certisfyCrypto.demoTrustAnchorFingerprint)))
        	return {error:`Claim is not trustworthy.`,claim,verification};
  
      const verificationResult = certisfyCrypto.getVerificationResult(verification,["pki-action"]);
...
router.post('/controller', async function(req, res, next) {
  try 
  {
      ...
      let verificationResult = certisfyCrypto.getVerificationResult(verification,["pki-action"]);
      ...
      if(action == "ppbv-create-agreement"){
           ...
           for(const field of verificationResult.unverifiedInformation.fields){
               let claimObject = certisfyCrypto.textToClaimObject(field.value)
               if(claimObject){
                    ...
                    claimObject = await certisfyCrypto.extractAndAttachPlainFields(claimObject,claim.plainFields);
                 
                    verification = await validateClaim(claimObject,action,CLAIM_TTL);
                    if(verification.error)
                      return res.status(200).json(verification);
                 
                    const partyVerificationResult = certisfyCrypto.getVerificationResult(verification)
          ...
          for(const party of parties){
              let expiryField = certisfyCrypto.getVerifiedCertificateField("pki-expiration-time",party.verification.fieldVerification.fields,true);
